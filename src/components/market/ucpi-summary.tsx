...
export function UcpiSummary({ index, snapshot, series }: UcpiSummaryProps) {
  return (
    <article
      ...
      className="flex flex-col gap-5 rounded-xl border border-neutral-200 bg-white p-5 sm:p-6"
    >
      <header className="flex flex-col gap-3">
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
          <h3 id="ucpi-heading" className="text-lg font-semibold tracking-tight text-neutral-950">
            {index.symbol}
          </h3>
          <span className="text-sm text-neutral-500">{index.name}</span>
        ...
        <p className="flex flex-wrap items-baseline gap-x-3 gap-y-1 tabular-nums">
          ...
          <span className="text-sm text-neutral-500">{index.unit}</span>{" "}

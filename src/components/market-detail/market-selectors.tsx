...
const selectClass = `h-10 w-full appearance-none rounded-md bg-transparent pl-3 pr-9 text-sm font-medium text-neutral-100 disabled:cursor-not-allowed disabled:text-neutral-500 [&>option]:bg-neutral-900 ${focusRing}`;
const fieldClass =
 ...
 * segmented market-family switch and the instrument select within that
 * family: together they decide what is being viewed. The comparison select,
 * limited to series that share the instrument's unit, sits on its own row
 * beneath them because it only modifies the current view. Families and
...
export function MarketSelectors({
  ...
  return (
    <div className="flex w-full flex-col gap-3 lg:w-auto lg:items-end">
      ...
      <div className={`${fieldClass} sm:min-w-64 sm:self-end`}>
        <label
          htmlFor="market-comparison"
          className="shrink-0 whitespace-nowrap pl-3 text-sm text-neutral-400"
        >
          Compare with
        </label>
        <select
        ...
        >
          <option value={NONE}>
            {comparisons.length === 0 ? "No compatible series" : "None"}
          </option>

...
export const SOURCE_INTERFACES: readonly UbwiSourceInterface[] = [
  ...
  // hash, and every `decisiveClause` is a verbatim slice of that document.
  {
    ...
    automatedRetrievalAvailable: true,
    note:
      "Section 20 (Explorer) of the Blockchain.com Terms of Service governs the Explorer API and grants access and use. It states no redistribution, retention or attribution terms, and scopes the service to informational purposes; it therefore does not grant the commercial derived-index publication Urdais performs. A separate 'API Terms of Service' is referenced in the site's own translation bundle but is served from no reachable path on www.blockchain.com (/legal/api returns HTTP 404), so it could not be retrieved and nothing is assumed from it. RETIRED AS A SUPPLY SOURCE BY METHODOLOGY 1.2.0. Phase 2E's note already observed that issued supply is a deterministic property of the chain, reproducible from the issuance schedule at a stated height, and that this interface supplied only its retrieval. Phase 2F acted on that: the supply quantity is now derived arithmetically from the reference height and no supply figure is read from here or from anywhere else. What remains is that blockchain.info/q/getblockcount is one of the two independent endpoints Urdais reads the chain-tip integer from. That is a different act on a different endpoint -- reading one consensus integer for cross-verification, deriving nothing from the provider's authority -- and the grant of access and use in Section 20 covers it. The retained artifact is kept as the evidence for the retirement, not as a licence Urdais relies on for any published quantity.",
  },
  {
    slug: "coinbase-spot",

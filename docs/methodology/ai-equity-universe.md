...
**Annual measure and interim evidence.** The default comparable measure is `Q_i / R_i` for a completed fiscal year, with numerator and denominator from the same reporting period and consolidation perimeter. Filed interim reports and reconciled issuer operating disclosures (tier 3 of the [Source Hierarchy](#docs-source-hierarchy)) do not replace that measure by default. They may: provide more recent evidence of business composition; identify a material acquisition, disposal, or change of consolidation perimeter; trigger a classification or eligibility review; and establish that the annual evidence previously relied upon is stale or structurally invalid. An interim disclosure may update the quantitative measure itself only where the qualifying numerator and the consolidated denominator are both available for the same interim period on a compatible, reconciled basis; the resulting ratio is then recorded with its period and audit status and supersedes the annual measure for that company's admission and retention decisions until a later compatible period is available. Where compatible interim data are insufficient, do not construct annualized quarterly revenue, pro forma exposure, management-estimate or guidance percentages, or analyst estimates. Retain the last valid comparable measure while its consolidation perimeter remains structurally valid and record the interim evidence separately against the company; where a material event has made the prior period non-comparable and no compatible measure exists, the determination becomes **insufficient evidence** under [Missing and pre-commercial exposure](#docs-missing-and-pre-commercial-exposure) and the review/removal policy applies.
...
**Event-snapshot weights.** An exceptional deletion produces a dated event snapshot with new canonical membership and base weights. Let `S` be the surviving membership, `w_i` the pre-event canonical base weight of surviving company `i` in the snapshot being superseded, and `c` the approved issuer cap. The event-snapshot weight is:

`w'_i = min(c, λ × w_i)` for `i ∈ S`, with `λ > 0` chosen so that `Σ_{i∈S} w'_i = 1`.

Equivalently: remove the deleted companies; take the surviving pre-event base weights as the proportional inputs; redistribute the deleted weight among survivors in proportion to those inputs; and reapply the issuer cap iteratively, redistributing any excess above `c` proportionally among uncapped survivors, until every weight satisfies the cap and the weights sum to one. Use unrounded values. No new capitalization, price, or float observation enters the event snapshot; it re-allocates the superseded snapshot's weights only. Because the cap can bind for survivors that were uncapped before the event, holding survivors in their pre-event proportions does not in general reproduce `w'`. The solution is unique when feasible. Feasibility requires a non-empty `S` and `|S| × c ≥ 1`; if either fails, do not relax the cap, add ineligible or replacement companies, or fall back to equal weights. Withhold the event snapshot, mark the universe unavailable from the event time, keep the superseded snapshot as historical with its original date, and publish the reason.
...
An event snapshot defines canonical parent membership and base weights for downstream consumers; it does not dictate downstream portfolio mechanics. A downstream methodology may consume `w'` at the event time under its own renormalization rule, or it may consume only the membership change. In particular, [UGAI](/docs/methodology/ugai) removes the deleted member with a divisor adjustment, leaves surviving index shares unchanged, lets its as-of weights continue to drift, and does not re-weight to `w'` before its next scheduled reset. Nothing in this section requires an unscheduled downstream rebalance.
...
UAVI begins with the canonical universe base weights, applies its own options-eligibility filter, and renormalizes the surviving constituents. It does not independently select an AI equity universe or replace the inherited weights. Options rules and calculations are left entirely to the UAVI methodology. The canonical base weights available for a given date are those of the scheduled snapshot effective on that date or of a valid event snapshot effective at that date (see [Corporate Actions and Exceptional Events](#docs-corporate-actions-and-exceptional-events)); which of these a downstream output consumes for a particular calculation date is specified by that output's methodology, not here. The parent guarantees that every published snapshot, scheduled or event, satisfies the sum-to-one and issuer-cap conditions.
...
- **Issuer cap c:** compare capped and uncapped distributions over point-in-time samples. Assess single-company and diversified-group dominance, effective constituent count, activity/country effects, small-issuer capacity, and infeasible cases, including event snapshots where `|S| × c < 1`. Approve a cap only if its representational benefit is defensible.

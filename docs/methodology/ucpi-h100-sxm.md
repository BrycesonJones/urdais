...
**Unresolved after this research**: the cross-time stability of the marketplace seller identifier, which binds only where a marketplace participates. The bundle envelope level, the seller-reduction rule, the freshness ages and the price carry limit were resolved by the 0.1.3 launch-readiness closeout, and the three numerical gates were closed by construction or converted to published diagnostics for the reasons given in [Publication Gates](#docs-publication-gates). Each is listed in [Launch Blockers](#docs-launch-blockers).
...
**The form of the envelope was resolved at 0.1.1. The level is declared below, at 0.1.3.**
...
### The level, and why it was not set at 0.1.1
...
**At 0.1.1 the concern was that choosing the level would be choosing whether this child publishes**, and that a level requires evidence of what host allocation is actually needed to use an H100 SXM for ordinary workloads, which price data cannot supply.

### The level, declared at 0.1.3

**The child declares a host-memory floor of 80 GB per accelerator, equal to the instrument's own device memory, and no floor on virtual CPUs or local storage.** An offer whose host memory per accelerator is below 80 GB is ineligible with reason `BUNDLE_OUT_OF_ENVELOPE`. Virtual CPU and storage allocations remain named accepted heterogeneity, retained on every observation and published as dispersion.

The anchor is chosen because it is the one host quantity that attaches to the instrument rather than to a workload assumption. A host that cannot hold a device-sized working set cannot stage the accelerator's full memory through ordinary host-side loading, which is the plainest reading of the family's phrase "too small to keep an H100 SXM supplied". No number above it can be derived from the instrument itself. Every professionally operated H100 SXM product observed across the research passes allocates at least 125 GB of host memory per accelerator, so the floor sits below the entire specialist segment and binds only in the marketplace segment. That answers the 0.1.1 concern: for the specialist clouds that form the launch candidate set the floor decides nothing, and where it does bind it reduces coverage rather than creating it, which is the opposite of the motive the family forbids.

The cost is stated rather than hidden. Across the 23 marketplace research candidates, host memory per accelerator ranged from 13.8 GB to 251.8 GB with a median of 55.4 GB, so this floor would exclude roughly half of that venue's candidates. Under the family's rationale for the envelope that is a genuine product distinction, and the child accepts it. The marketplace experiment also found that within-cell price did not track host memory in either direction, so the floor is not expected to move the level where it does not bind, and it was not chosen for any effect on the level.

The floor is a declaration of what this child treats as ordinary, anchored to the instrument; it is not a measured workload threshold. A workload study could move it, and any change applies prospectively under the family's versioning rule. Synthetic decomposition remains prohibited: no imputed price is subtracted for virtual CPUs, memory, storage, or network.

**The family's requirement that a child declare an envelope before launch is met.**
...
### The decision at 0.1.1, and its terms

At 0.1.1 the seller minimum was retained as the family default, provisionally and unratified: six cells, at one venue, at one instant, in one country that reached the structural floor, were not grounds for ratifying a family-level rule. The draft set the terms of ratification: compare the minimum, the median and a canonical selection across several venues, include specialist-cloud cells rather than marketplace cells alone, and measure enumeration completeness per venue.

### The decision, ratified at 0.1.3

The launch-readiness closeout added the specialist-cloud evidence the marketplace experiment lacked, and it changed the answer.

**One specialist cloud publishes the same H100 SXM product at four quantities within the per-accelerator class, at four different per-accelerator prices: $4.29 for one accelerator, $4.19 for two, $4.09 for four and $3.99 for eight, read from its price surface on 13 September 2026.** The discount is monotone in quantity and spans 7.5%. Under the seller minimum that participant's observation would be $3.99, a price a buyer obtains only by taking eight accelerators at once. Under the seller median it would be $4.14, a price no buyer pays at all. For a child whose unit is one accelerator-hour, and whose class is defined by the buyer *not* being required to take a node, the minimum systematically selects the bulk tier at any seller with tiered pricing and so converts a per-accelerator measure into a best-bulk-rate measure. That is a defect of the rule, not of the sample, and it holds on any day the tiers differ. Where quantity variants are priced linearly, as the marketplace's machine partitions were to within 0.2%, the choice is immaterial; where they are not, it is the whole difference.

**The child therefore adopts a canonical-quantity selection with the family's minimum applied within it.** For each seller, in each cell, the eligible offers are restricted to those at the smallest accelerator count that seller offers within the per-accelerator class, which is one accelerator wherever a seller sells singly. The representative price is then the lowest currently accessible qualifying price among the remaining variants at that quantity, across tiers, zones and machines. Stated in the family's terms: a fixed selection on the quantity dimension, then the seller minimum.

The rule answers the economic question directly. It is the accessible price of the specified unit from that participant, at the quantity the specification measures, through the cheapest channel at which that quantity is actually obtainable. It resists catalogue duplication on the dimension where duplication was shown to bias the minimum, because a seller cannot lower its observation by adding larger-quantity variants. It keeps the family's accessibility logic on every remaining dimension, so the cheaper of two tiers or zones at the canonical quantity is the observation. And it is deterministic and reproducible from retained fields: accelerator count, variant, price.

Two exposures remain and are stated. The minimum across tiers and zones at the canonical quantity still carries the machine-count breadth bias measured at the marketplace, and still carries the one-sided enumeration bias on any venue whose population is incomplete. And the multi-day comparison the 0.1.1 draft called for has not been run, because no repeated collection exists; it moves to the validation programme as monitoring, since the argument above is structural rather than statistical and does not depend on it.
...
The parent names its third permitted alternative as "a fixed or canonical-zone selection"; this child applies the fixed selection to quantity. That reading is recorded in [Findings for the Parent Methodology](#docs-findings-for-the-parent-methodology) so the family can confirm or correct the wording.
...
- **Storage that the buyer sizes and that the seller bills separately from the accelerator rate** depends on how the buyer uses the product and is **outside** the price, recorded as a usage-dependent charge. Where a seller requires some non-zero allocation for the product to run at all and does not document the minimum, that requirement is recorded in the fee interpretation with its materiality bound; it is never imputed into the price.
- **A prepayment or credit-balance requirement** that must be met before deploying is a commercial constraint, recorded as a minimum-spend field under the family's rule, and is not a charge.
...
At 0.1.1 `price_max_age`, `availability_max_age` and `reference_data_max_age` were left unresolved on the ground that any value proposed then would be invented.

**They are resolved at 0.1.3 without a cadence study, because the resolution does not require one: both evidence ages are set to the calculation cycle itself, and no price or availability evidence is carried across calculation dates at launch.** An observation enters a calculation date's value only if its price and its availability evidence were both observed within that date's collection window, which the family defines as the half-open interval from `00:00:00 UTC` on the calculation date to `00:00:00 UTC` on the next; the last complete reconfirmation before that cutoff is the observation, and retries within the window are permitted and never extend it. `reference_data_max_age` is governed by version validity, as the family permits. The 0.1.1 ordering constraint holds with equality. No number was invented because there is none: this is a zero-carry rule, and the cadence study that would justify a longer carry is the condition for ever relaxing it, prospectively.
...
- **Price** is not carried at launch. `price_max_age` equals the calculation cycle, so a price not re-observed on the calculation date makes the observation ineligible with reason `PRICE_STALE`. The `PRICE_CARRIED` diagnostic and its age field remain defined for the day a cadence study supports a bounded carry, and are unused until then.
...
The cases are therefore mechanical. Price and availability both re-observed on the calculation date: eligible, with freshness Reconfirmed or Newly observed. A source unreachable at every attempt before the cutoff: Source unavailable, the participant leaves that date's coverage, and the change in participant count and, where it follows, in market breadth is disclosed. A fresh price with availability not re-observed, or fresh availability with a price not re-observed: ineligible under `AVAILABILITY_STALE` or `PRICE_STALE` respectively; for the specialist-cloud interfaces studied this combination cannot arise, because each returns price and availability in one response. Both stale: ineligible. Nothing is carried silently, because nothing is carried. Where a region has two participants, a single source failure on a date produces Unavailable for that date rather than a value resting on yesterday's accessibility; the collector's mitigation is to retry within the window before the cutoff, which is an implementation matter and not a carry.

Weekends and holidays are ordinary calculation dates, as the family already states. The 0.1.1 question arose from page-updating behaviour and does not apply to machine-readable interfaces, which answer on any day they are asked.
...
- **Maximum out-of-envelope bundle share: zero**, because an offer outside the envelope is ineligible rather than adjusted under the family's rule, and the level is declared in [Bundle Envelope](#docs-bundle-envelope).
- **Maximum carried share: zero.** No price or availability evidence is carried at launch, so the carried share is zero on every calculation date.
...
### Open at 0.1.1, resolved at 0.1.3

Two of the three gates that remained open numerically at 0.1.1 are converted to published diagnostics, and the third closed by construction above. The reasoning is given in full because a gate becoming a diagnostic must never be a convenience.

- **Share resting on the weakest admitted availability grade, and share resting on Limited rather than Available: published diagnostics, not gates.** Grade 2 evidence exists at one venue in this market and that venue is blocked; Grade 3 is this child's minimum and is what the specialist segment exposes. A share gate on Grade 3 would therefore be a gate on venue mix, which is the impossible-gate reasoning already applied to operator attribution. There is a second and more general reason. At two or three participants every share is 0%, 33%, 50%, 67% or 100%, so a share gate is a participant-count gate in disguise, and the family has just decided that the participant count qualifies a value rather than suppressing it. Both shares are published with every value. A numerical gate may be introduced by amendment once participant counts are large enough for a share to measure composition rather than count.
- **Maximum `TAX_BASIS_UNRESOLVED` share: published diagnostic, not a gate, for the same participant-count reason.** The launch candidate set does not exercise it: both specialist clouds studied establish a tax-exclusive basis from their general terms, one stating that prices are shown plus applicable sales tax, VAT and GST, the other that its fees do not include any sales tax that may be due. The risk the gate existed to bound is disclosed by the share, and a gate may be introduced by amendment when the share can measure composition.
...
**No numerical publication gate remains open for launch.** Every gate the family names is either closed by construction, resolved structurally by the family's market-breadth rule, or published as a diagnostic with the reasoning above. None was set from a sample assembled to test the methodology, because none was set to a number at all.
...
- the prior observation, meaning the immediately preceding calculation date and never an earlier one, was **Delayed or Unavailable**, so there is no prior published level to compare against;
...
**Bundle.** Host memory per accelerator: **nullable but blocks P2**, since the envelope floor is declared. Virtual CPU allocation, local storage, and intra-node and inter-node interconnect, each per accelerator: optional diagnostic, published as dispersion.
...
- **Seller reduction.** Evidence: six comparable cells recovered for the first time; median above minimum by up to 18.95% and 3.71% on average; the regional value was unchanged in the one region reaching the structural floor, which is coincidence at three participants; catalogue-breadth bias confirmed with machine count as its driver; the venue's interface cannot enumerate its own population, which biases any minimum upward. Decision at 0.1.1: family default retained, provisionally and unratified, with a new enumeration diagnostic. **Decision at 0.1.3: ratified as a canonical-quantity selection with the seller minimum applied within it**, after a specialist cloud's public price surface showed a monotone 7.5% quantity discount within the per-accelerator class, under which the bare minimum would select the eight-accelerator tier for a one-accelerator unit. Confidence: high on the structural argument; the multi-day comparison moves to monitoring. Launch blocker: **no**.
- **Bundle envelope.** Evidence: marketplace dispersion of 7.0, 18.3 and 8.7 times against 1.62 and 2.05 among disclosing specialist clouds; a two-sided band drawn from the latter admits 9% of the former; a floor at 16 virtual CPUs and 125GB takes the one qualifying country to a single participant; every professionally operated product observed allocates at least 125 GB of host memory per accelerator. Decision at 0.1.1: form resolved as a one-sided floor, level unresolved. **Decision at 0.1.3: host-memory floor of 80 GB per accelerator, equal to device memory, with no virtual CPU or storage floor.** Confidence: medium; it is a declaration anchored to the instrument, not a measured workload threshold, and it excludes roughly half the marketplace candidates while binding on no specialist cloud. Launch blocker: **no**.
...
- **Numerical freshness ages.** Evidence: 100% of one hyperscaler's current H100 prices had stood at least 196 days, median 955 days, changing only on month boundaries; specialist clouds expose no effective dates; availability observed at one instant. Decision at 0.1.1: unresolved, with one ordering constraint adopted. **Decision at 0.1.3: both evidence ages equal the calculation cycle; nothing is carried across dates at launch.** Confidence: high, because the rule contains no estimated number; relaxing it requires the cadence study. Launch blocker: **no**.
- **Carry.** Evidence: availability can change with no price change, and is offer-addressable; the specialist-cloud interfaces studied return price and availability in one response. Decision: reference data carried on version; **neither price nor an Available state is carried at launch**, with the operational cost stated and retry-before-cutoff named as the mitigation. Confidence: high. Launch blocker: **no**.
- **Tax basis.** Evidence: two specialist clouds establish a tax-exclusive basis from their general terms; most other sellers silent. Decision: resolved, evidence by general terms; an established inclusive basis is disqualifying; an unestablished basis is flagged and eligible, with its share published as a diagnostic rather than gated, for the participant-count reason in [Publication Gates](#docs-publication-gates). Confidence: medium-high. Launch blocker: **no**.
...
- **Calculation calendar.** Evidence: a family-level convention, decided in UCPI 0.1.2-draft. Decision: inherited unchanged; the child's zero-carry and re-observation rules are stated within the family's UTC window. Confidence: high. Launch blocker: no.
...
- **Cross-time stability of the marketplace seller identifier.** Requires repeated observation of the same venue over days, testing whether an identifier persists, whether one seller holds several, and whether one survives a hardware change. **Binds only for marketplace participation**; a launch resting on specialist clouds alone does not depend on it.
- **Minimum purchasable topology for the specific sellers where no source field has yet been read**, which for the key-gated specialist clouds is established at the first authenticated call to the documented field, and for the unresearched sellers requires the source study.

**Resolved at 0.1.3 and no longer listed**: the seller-reduction rule, ratified as a canonical-quantity selection with the seller minimum within it; the bundle envelope level, declared as an 80 GB host-memory floor per accelerator; the freshness ages and the price carry limit, set to the calculation cycle with no carry at launch; and the three numerical gates, one closed by construction and two published as diagnostics with the reasoning in [Publication Gates](#docs-publication-gates). The multi-day seller-reduction comparison and the cadence study move to the validation programme as monitoring, because the launch rules do not depend on their outcome.
...
- **Per-source eligibility evidence that only the source can supply.** For each candidate seller: the minimum purchasable accelerator count from the documented field; the mapping of each datacenter or region identifier to a country, using the source's own country designation where it exposes one and never the identifier's spelling; and, for any seller whose official documentation is silent on exclusivity, a statement establishing Explicit or Documented tenancy, without which its observations are `TENANCY_UNRESOLVED` and never reach P2. These are source-registry items recorded in the internal source documents, not methodology parameters, and none can be closed by analysis.
...
**Requires a parent decision**: none. Both requests made at 0.1.3 were answered in UCPI 0.1.2-draft. The family confirmed that its third seller-reduction alternative extends to a fixed selection on the quantity dimension, so the rule ratified above stands on the parent's own wording. And the family now owns the calculation calendar: a UTC calendar date with a half-open window from midnight to the next midnight, the cutoff at the next midnight exclusive, Urdais's observation time controlling eligibility, retry within the window, publication after validation with a deadline at the following cutoff, and every calendar date a calculation date. This child inherits that calendar unchanged; its zero-carry rule, its availability re-observation rule and its percentage-change conditions are all stated within it.

**Study limitation worth stating plainly.** The 0.1.3 closeout rests on the public price surfaces, published interface specifications and product documentation of two specialist clouds, read on one day, with no authenticated call made; the 0.1.1 empirical work rests on **one marketplace at one instant**, plus published specifications and unauthenticated catalog interfaces at four other providers. The marketplace's hosts are individual accounts rather than companies, its bundles are far more heterogeneous than the specialist-cloud segment, and its interface cannot enumerate its own population. Findings drawn from it are stated as findings about one venue, and none is presented as a market-wide rate.
...
**0.1.4-draft, 13 September 2026**: calendar inheritance. Adopts the family's calculation calendar from UCPI 0.1.2-draft without addition, states the child's freshness rule within the family's UTC window, fixes the percentage-change predecessor as the immediately preceding calculation date, and withdraws both parent requests made at 0.1.3 as answered. No rule of this child changes. No provider is named, approved or reclassified; P2 remains empty; launch remains blocked on permission, on per-source evidence, and on collector implementation. No production effective date.

**0.1.3-draft, 13 September 2026**: launch-readiness closeout. Resolves every remaining methodology parameter that does not depend on a provider's permission or on a source's own statement. The seller-reduction rule is ratified as a canonical-quantity selection with the seller minimum applied within it, after one specialist cloud's price surface, whose size selector the 12 September snapshot could not read, showed the same product at $4.29, $4.19, $4.09 and $3.99 per accelerator-hour for one, two, four and eight accelerators, a monotone 7.5% discount under which the bare minimum would select the eight-accelerator tier for a one-accelerator unit. The bundle envelope level is declared as a host-memory floor of 80 GB per accelerator, equal to device memory, with no virtual CPU or storage floor, binding on no specialist cloud and on roughly half the marketplace candidates. The freshness ages are set to the calculation cycle with no price or availability carry at launch, and weekends are confirmed as ordinary calculation dates. Buyer-sized separately billed storage is placed outside the price and a prepayment requirement is recorded as a commercial constraint. Of the three numerical gates, the carried share is closed by construction at zero and the weakest-grade, Limited and unresolved-tax-basis shares become published diagnostics, on the ground that at two or three participants a share gate is a participant-count gate in disguise. Two small parent confirmations are requested and no parent rule is changed. No provider is named, approved or reclassified; P2 remains empty; launch remains blocked on permission, on per-source evidence that only the sources can supply, and on collector implementation. No production effective date.

...
**Requires a parent decision**: two, both small and neither a rule change. First, confirmation that the family's third seller-reduction alternative, worded as "a fixed or canonical-zone selection", extends to a fixed selection on the quantity dimension as this child applies it; the child reads "fixed selection" as covering it and asks the family to say so in its wording. Second, the calculation cutoff and publication target, which the family lists as unresolved and which a collector needs before its first run; this child proposes the end of the calculation date in UTC as the cutoff and does not adopt it, because the parameter is the family's.

...
**Version 1.3, 22 September 2026.** Status: in production.

Urdais publishes Token Price for Anthropic, OpenAI and xAI, computed under version 1.1 and live since 14 September 2026. Google, Alibaba Cloud and Moonshot AI are verified and entered under version 1.2. DeepSeek is collected but withheld, for the reason recorded below. Version 1.3 changes one thing: xAI's designated model becomes Grok 4.7.
...
- **xAI**: Grok 4.7 (`grok-4.7`), from 22 September 2026. The manual review of that date found Grok 4.7 published as the current general-purpose Grok, at $2 input and $6 output under 200,000 prompt tokens. Grok 4.6 (`grok-4.6`) remains designated for 14 to 21 September 2026 and its values are unchanged. Grok Build 0.1 is coding-specific, the Grok 4.20 multi-agent build is agent-specific, and the 4.20 reasoning and non-reasoning entries are mode-specific variants of an earlier version.

  The two models are published at the same pair, so the benchmark is $4.00 on both sides of the boundary. That is not a reason to leave the designation where it was. The designated model is defined as the provider's current broadly available flagship, and when the provider moves it, the measurement moves with it or it stops measuring what it says it measures. Percentage change is withheld across the boundary all the same, under the constituent-change rule below: two equal numbers computed from two different models are not a zero change, they are two different economic objects.
...
**1.3, 22 September 2026**: redesignates xAI to Grok 4.7, effective 22 September 2026, on the manual review of that date. The standardized workload, the weights and every other designation are unchanged, and no value computed under an earlier version changes: xAI's Grok 4.6 segment keeps the values it had, and the Grok 4.7 segment begins at its own effective date. The published pair is $2 input and $6 output under both designations, so the calculated benchmark does not move across the boundary; the designation does, and percentage change is withheld across it.

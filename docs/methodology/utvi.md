...
**Status: draft, version 0.1.0-draft, no production effective date.** Prepared 16 September 2026. **Nothing may be published under this version.** It is a draft in the sense the registry means: a methodology version with no effective date, which the database will not allow a publication to reference.

This draft rests on the [UTVI Phase 1 source study](../research/utvi/source-study.md) and is accompanied by the [data-architecture proposal](../architecture/utvi-data-architecture.md). Its purpose is to state what UTVI would measure, precisely enough that the decision to build it can be taken on evidence — and precisely enough that the decision *not* to build it remains available.
...
Under this version the observed universe is **one marketplace**, and the evidence in the source study puts it at roughly **one per cent of world token throughput**. A measure of 1 % of a market is a legitimate market-data product — many are — but only if it says so in the same breath as its value.
...
It is a **demand-side consumption quantity**, the volume counterpart to [Urdais Token Price](token-price.md)'s unit cost. Price says what a token costs; UTVI says how many were consumed where Urdais can see. Together they describe a market in the way that price and volume describe any other.

What UTVI is **not** an estimator of, and must not be presented as a proxy for:

- **World token consumption.** It is a sample of known, non-random, ~1 % coverage, with a known direction of bias. A sample like that does not scale up.
...
| Included | Excluded |
|---|---|
| Traffic routed through the observed platform | First-party API traffic direct to a lab |
| Public models on that platform | Consumer assistants (ChatGPT, Gemini app, Doubao) |
| Both proprietary and open-weight models it serves | Self-hosted and on-premise inference |
| All modalities the platform serves | Other routers, gateways and clouds |
| | Training and fine-tuning of any kind |
...
**Normalisation does not fix what it appears to fix.** The argument for base-100 is that the level invites a false comparison with world consumption. But re-basing hides the coverage problem rather than solving it: a reader told "UTVI = 143.2" has *less* ability to notice that the figure is 1 % of the market than a reader told "3.61 T tokens/day", who can compare it to a disclosure. The coverage problem is solved by disclosure and by the word *Observed*, not by changing units.
...
Under 0.1.0-draft **no source is production-approved**, so no value may be calculated. The candidate is OpenRouter's Datasets API, whose two rights axes both read permitted; see [the terms note](../architecture/sources/openrouter-datasets.md).
...
|---|---|---|
| Input / prompt tokens | **Included** | |
| Output / completion tokens | **Included** | |
| Reasoning / thinking tokens | **Included** | The source reports them inside completion tokens; not separable, and stated rather than hidden |
| Multimodal image and audio tokens | **Included as reported** | No image↔text equivalence is asserted or computed |
| Cached input tokens | **Undetermined** | Presumed inside prompt tokens; not asserted until answered |
| Batch inference | Included if present | Not separable |
| Embeddings | **Presumed out of scope** | Undetermined until answered |
| Training and fine-tuning tokens | **Excluded** | A different economic object: production of a model, not consumption of one |
| Internal hidden tokens | Out of reach | Not claimed |
...
|---|---|---|
| **Source model id** | The platform's own identifier, stored verbatim, never rewritten | `anthropic/claude-3.5-sonnet-20241022:free` |
...
**Stable identity is the source's own identifier.** Display names change; marketing renames a model; a lab re-brands. None of that may rewrite history, and the rule is [`identity.ts`](../../src/lib/tokens/identity.ts)'s already: the key is `(namespace, native id)`, and an identity survives a rename by construction.
...
**Variants fold for attribution and not for the total.** `model:free` and `model` are one canonical model with two source ids. Folding them is required for model- and lab-level attribution, and is irrelevant to the daily total, which sums all traffic once either way.
...
3. **Percentage change is withheld across the break** — never computed between two different economic objects, and never shown as zero. This is exactly the rule [Token Price](token-price.md#constituent-changes) applies at a designation change, reused rather than reinvented.
...
One scheduled job per UTC day, reading the most recent completed UTC day — the pattern [`daily-run.ts`](../../src/lib/ucpi/runtime/daily-run.ts) already implements, where a run shortly after midnight UTC collects for the day that just closed.
...
5. **The settlement lag is a parameter of this methodology version and is left unset in 0.1.0-draft**, because it must be measured — read the same date daily for two weeks and record the deltas — and cannot be measured without source access.
...
- the **unattributed residual is published beside the shares**, as its own figure;
- it is neither silently dropped from the denominator nor silently folded into a named lab;
- a share is stated as a **share of attributed tokens**, and never as a share of the total unless the residual is zero.
...
The candidate source retains daily history from **2025-01-01** and rejects earlier dates, so a ~20-month backfill is available in one pass. That is enough for every period in §15 — including 1-year change — from the first print. Backfilled points are marked as such and settle under §11 like any other.

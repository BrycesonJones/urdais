...
**Status: proposed methodology, version 0.1.2-draft.** Prepared 12 September 2026, amended 13 September 2026. No production value, child specification, provider list, parameter set, or price history is established by this document. Nothing here describes live infrastructure.
...
### The calculation calendar and cutoff

The family owns the calendar. A child inherits it and may depart from it only by an explicit, versioned amendment stating why its instrument's market requires a different convention. The first child inherits it unchanged.

**A calculation date is a UTC calendar date `D`, and its collection window is the half-open interval `[D 00:00:00 UTC, D+1 00:00:00 UTC)`.** The calculation cutoff for `D` is the instant `D+1 00:00:00 UTC`, exclusive: an observation at exactly that instant belongs to `D+1`. There is no separate end-of-day time to interpret and no local time zone anywhere in the rule.

**The controlling timestamp is Urdais's own observation time**: the moment the source's response was received and recorded, and for availability evidence the moment that evidence was re-observed. A source-effective time never controls eligibility; it is retained where disclosed and is frequently absent. A request that begins before the cutoff and completes after it produces an observation in the next window, because the evidence did not exist before the cutoff, and a response received after the cutoff carrying a source timestamp from before it likewise belongs to the next window. This is the look-ahead prohibition made mechanical: a value for `D` uses no input observed at or after `D+1 00:00:00 UTC`.

**Collectors may retrieve and retry throughout the window.** Where a source is observed more than once in a window, the observation that enters the calculation is the **last complete reconfirmation before the cutoff**: the latest retrieval in the window at which every eligibility-relevant field was re-established. Earlier retrievals are retained as evidence and are never averaged. A source not successfully observed at any point in the window is Source unavailable for `D`, and its participants leave that date's coverage under [Missing Data](#docs-missing-data). Retries are an operational matter and never extend the window.

**The daily observation therefore represents the most recent eligible state of each participant successfully reconfirmed within the window for that date.** It is neither a snapshot at one instant nor an average over the day.

**Calculation begins at the cutoff and uses only inputs observed within the window.** It is reproducible: re-running the calculation for `D` at any later time from the retained inputs yields the same value, and no wall-clock time enters the result except the cutoff itself. Validation follows under [Data Quality Rules](#docs-data-quality-rules). Each observation carries its calculation date `D`, the cutoff as its as-of instant, the time it was calculated, and the time it was published, and these are the calculation and publication timestamps already required of every observation.

**Publication follows validation as soon as it completes, and no later than the next cutoff.** The publication deadline for `D` is `D+2 00:00:00 UTC`. A value released before the deadline has status Published. A value not released by the deadline has status Delayed until it is released, and when released it carries its calculation date `D` and its actual publication time, so a late print is never presented as timely and a user always sees the calculation date separately from the publication time. A value that cannot be produced for `D` is Unavailable for `D`, and nothing is filled in.

**A percentage change is computed against the immediately preceding calculation date only.** Where that date has no Published level, the change is withheld rather than computed against an earlier date; a child states the further conditions under which it is annotated or withheld.

**Corrections are unchanged.** An input later found wrong for `D` is handled under [Corrections and Restatements](#docs-corrections-and-restatements): the recalculation uses the same window and the same retained inputs with the corrected one, the restated value carries the original and the reason, and the original is retained as Superseded. A percentage change is a derived quantity of two published levels and follows the correction status of its endpoints. The cutoff never moves.

**Every calendar date is a calculation date.** Saturdays, Sundays and holidays use the same window and cutoff. A price that has not changed is fresh if it was reconfirmed within the window; a price that was not reconfirmed within the window is not fresh however recently it last changed; and availability must itself be observed within the window, as [Freshness and Staleness](#docs-freshness-and-staleness) requires. No market calendar applies, because this market does not close.

The publication deadline is the one parameter here that may be tightened once validation runtimes are measured; it may be shortened by amendment and is not to be lengthened to accommodate a slow pipeline.
...
The first child must therefore empirically compare at least three rules before the family adopts one: the **seller minimum** defined above; the **seller median** across eligible offers in the cell, which is less sensitive to a single cheap zone; and a **fixed or canonical selection** on a variant dimension, for example a stable zone where one can be identified or the seller's smallest offered quantity, which removes the breadth asymmetry on that dimension entirely and may be combined with the minimum across the remaining variants. A **capacity-weighted price within the cell** is a fourth candidate wherever capacity is disclosed. The data may not vindicate the minimum, and the family does not assume it will.

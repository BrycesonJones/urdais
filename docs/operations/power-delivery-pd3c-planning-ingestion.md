...
Provenance is pinned at two levels, kept separate. `archive_ref` carries the SHA-256 of the whole retrieved artifact, which is also the retrieval's own hash. `archive_member_hash` carries the SHA-256 of the decompressed bytes of the specific member the parser read — so two sheets of one workbook are two members with two different hashes, and a value can be traced to the part it came out of rather than only to the package that contained it. Both are queryable on `pipeline.raw_planning_forecast_records`, alongside `archive_member`.

Members are verified before they are parsed. `readZipMember` checks the decompressed length against the size the central directory declares and the CRC-32 against the stored one, and raises `ZipIntegrityError` naming the member if either disagrees. A member whose size lives in a ZIP64 extra field is refused outright rather than sliced on the 32-bit sentinel and misread. A truncated download or a mangled byte therefore stops the run instead of quietly becoming fewer rows, or subtly wrong ones.

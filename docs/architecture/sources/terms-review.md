...
**Runpod asked and was refused.** That reasoning made Runpod the most actionable target in the set, and it was acted on: the permission request went out on 13 September 2026 and Runpod refused both permissions on 14 September 2026, declining to approve systematic retrieval for the purpose of building a compilation or use of the data in a commercial market-data product. The remedy the Terms named has been sought and spent, so Runpod now heads nothing: it is closed as a direct production source under the current methodology, and reopening it needs a materially different intended use and a fresh approval. Its availability data remains the strongest found anywhere, and remains unavailable. See `runpod-permission-denied.md`. Note that REST API v1 is deprecated and retires on 15 November 2026; the endpoint recorded in the registry is the v2 path, `GET /v2/catalog/gpus` on `https://api.runpod.io`.
...
3. **Vast.ai, third. Request sent 13 September 2026, awaiting a response.** Its terms exclude Urdais most directly of the three: a licence whose two enumerated purposes do not include market-data products, an express exclusion of *"any other commercial data product or market-information purpose"*, and a clause naming indices and benchmarks. Vast has also reserved index and data-product rights in the same data to itself, so the request asks for something the provider wants to keep. Expect a refusal and treat it as a result. Separately, the enumeration limitation Phase 2 measured means **permission alone would not make this source usable**, so a positive answer would need the enumeration question resolved before any collector work. Vast therefore has **two independent gates**, and failing either keeps it out of UCPI. The request, the route chosen and the evidence of sending are recorded in the Vast.ai Agreement Request document held alongside this one.

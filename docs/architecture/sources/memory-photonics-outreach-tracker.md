...
|---|---|---|
| iccsz.com (讯石光通讯网) | Photonics | Publishes China Telecom / China Mobile tender results with unit counts and winning bidders — among the few public sources of near-transaction pricing in optics. robots.txt not fetched, terms not read, and it is unconfirmed whether a price index is published. **Highest-value unexplored lead in either vertical.** |

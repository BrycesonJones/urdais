...
| nebius | 4.50 | Nebius (legal name unevidenced) | Phase 1: per-GPU-hour on-demand listing | eligible |
...
Five independent sellers survive. The sibling's candidate value is the median of {1.99, 3.49, 3.99, 4.09, 4.50} = **3.99**, Normal breadth, participant count 5, technical-source count 1 (largest-source participant share 1.0). The vendor's own on-demand median across its 11 rows was 3.87; Urdais does not use it.
...
Under the LISTED sibling: 5 eligible (Voltage Park 1.99, Runpod 3.49, Hyperstack 3.99, Lambda 4.19, Nebius 4.50); 11 excluded for the reasons in the table above (the procurement-mode, whole-node, topology-unknown and marketplace-aggregate rules; every exclusion carries a named code). Candidate value **3.99**, Normal breadth, participant count 5, contributing technical sources 1, largest-source participant share 1.0, dispersion p10 2.59 / p50 3.99 / p90 4.376 / IQR 0.70, one-day change withheld (no prior). Attribution: *Data: Price of Compute — priceofcompute.com*. The vendor's headline 3.93 is not Urdais's number and is not used.

...
This is a **candidate**, not a run and not a publication. The family calendar closes the 2026-09-14 window at 2026-09-15T00:00Z, and the database refuses a calculation run dated before its cutoff; the run is recorded by `scripts/ucpi/listed-print.ts calculate` after that time, as a simulation run while the family methodology and the sibling specification are drafts. No `regional_publications` row can exist until both carry approved versions.

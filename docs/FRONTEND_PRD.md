...
The `/map` route is a dedicated map workspace so the heavier map renderer stays isolated from the rest of the product. Its basemap (OpenFreeMap Positron with Urdais label overrides) provides city-level geographic context that densifies with zoom, plus a metric distance scale, and it renders points through one GeoJSON source and one circle layer (currently a static demo set; production and backend data remain deferred). Points carry a mapped/unmapped status and mapped points carry an infrastructure category (Data Center, Compute Cluster, Power Infrastructure, Semiconductor Fab): mapped points are coloured by category, unmapped points render black, and a small legend lists the four categories and Unmapped. Each legend row is an interactive visibility filter: categories and Unmapped toggle independently, all groups are visible by default, and the filter state resets on refresh (no persistence yet). The GeoJSON source uses MapLibre's native clustering: dense areas render as neutral cluster circles with point-count labels, clicking a cluster zooms to where it splits, and clusters only ever count currently visible groups because the source is fed the visible subset; advanced clustering behaviour, advanced filtering, and subtypes remain deferred. Clicking a mapped point opens a compact profile popup that prioritises name, category, address, and public contact email (as a mailto link) whose metadata is still demo/static; richer item profiles and backend data remain deferred, as do category-specific colours, category colours, legends, popups, filtering, clustering, and item detail routing are intentionally deferred to later phases.

...
- **Indices:** to be developed one output at a time. The first proposed index methodology is [UGAI](/docs/methodology/ugai), the Urdais Global AI Index, which consumes the shared AI Equity Universe. [UAVI](/docs/methodology/uavi), the Urdais AI Volatility Index, is a proposed sibling output that consumes the same universe and measures 30-day option-implied volatility across its option-eligible members. [UBWI](/docs/methodology/ubwi), the Urdais Bitcoin Wealth Index, is a separate proposed index that shares no universe with either: it measures Bitcoin's market capitalization as a percentage of Total Global Wealth, and its value is a percentage rather than an index level. [UTVI](/docs/methodology/utvi), the Urdais Observed Token Volume Index, is a production methodology for observed model token consumption per day, approved at 1.0.0 and publishing since 16 September 2026. It shares no universe with the others: it measures the token volume one marketplace's dataset exposes, and its universe statement defers to that source rather than asserting what the source does not document. [Market Share](/docs/methodology/market-share) is its derived breakdown, versioned with it at 1.1.0: share of that same observed volume by model and by canonical lab, with both residuals reported rather than absorbed.
...
- **Markets:** to be developed one output at a time. The first is [UMPI-KR DRAM](/docs/methodology/umpi-kr-dram), version 1 of the Urdais Memory Price Index: two monthly official-data DRAM indexes built from Korean government statistics, in index points, with a MoM change. Its predecessor [UMPI-DRAM Spot](/docs/methodology/umpi) — six DRAM chips priced daily in USD per chip from a commercial spot board — is **deferred** rather than withdrawn: the definition is complete, the rights are not, and no open source prices those instruments at all.

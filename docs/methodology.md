...
- **Markets:** to be developed one output at a time. The first is [UMPI-DRAM Spot](/docs/methodology/umpi), version 1 of the Urdais Memory Price Index: the spot-market price of six defined DRAM chips, in USD per chip. It is a draft, and its publication state is blocked pending a licensed source — the definition is complete, the rights are not.
